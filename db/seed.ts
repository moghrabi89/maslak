...
async function main() {
  ...
  await db.delete(challengeAnswers);
  await db.delete(challengeSessions);
  await db.delete(reviewQueue);
  await db.delete(userProgress);
  await db.delete(userBadges);
  await db.delete(contentReviews);
  ...
  // Ensure system "seed" user exists for content review reference audits
  await db.insert(users).values({
    id: "seed",
    name: "مراجعة النظام التلقائية",
    email: "seed@maslak.app",
    role: "admin",
    xp: 0,
    gems: 0,
    streak: 0,
      ...
      data: {
        ...
        applyScenarios: [
          ...
          },
          {
            ...
            distractors: [
              "يعتبر دم استحاضة وعليها الوضوء فقط",
              "تنتظر حتى تتم 15 سنة ليحكم ببلوغها",
              "تعتبر طاهرة وتستمر في الصلاة والصيام"
            ]
          },
          {
            ...
            distractors: [
              "يعتبر بالغاً فور رؤية البلل",
              "يجب عليه إعادة الصلاة منذ تاريخ الادعاء",
              "يقبل قوله وتجب عليه الواجبات احتياطاً"
            ]
          }
        ]
      }
      ...
      data: {
        ...
        applyScenarios: [
          ...
          },
          {
            ...
            distractors: [
              "وضوؤه صحيح تماماً ويكتفى بنية غسل الكفين",
              "يصح وضوؤه وتلزمه فدية إهمال النية عند غسل الوجه",
              "لا يصح وضوؤه ويجب عليه إعادة الوضوء بالكامل فوراً"
            ]
          },
          {
            ...
            distractors: [
              "يصح المسح وتكره الصلاة به",
              "يصح المسح ويستحب له إعادة المسح على البشرة",
              "يصح الوضوء وتجب إعادة الصلاة فقط"
            ]
          }
        ]
      }
      ...
      data: {
        ...
        applyScenarios: [
          ...
          },
          {
            ...
            distractors: [
              "يصح وضوؤه ولكنه يكره",
              "يصح وضوؤه وتلزمه إعادة الصلاة فقط",
              "يصح وضوؤه ويصلي به النافلة دون الفرض"
            ]
          },
          {
